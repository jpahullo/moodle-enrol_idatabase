...
/**
 * Improved database enrolment plugin.
 *
 * This plugin synchronises enrolment and roles with external database tables.
 *
...
class enrol_idatabase_plugin extends enrol_plugin {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol_user($instance, $ue) && has_capability('enrol/idatabase:unenrol', $context)) {
    ...
    public function sync_user_enrolments($user) {
        ...
        if ($rs = $extdb->Execute($sql)) {
            if (!$rs->EOF) {
                while ($fields = $rs->FetchRow()) {
                    ...
                    if ($instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'idatabase'), '*', IGNORE_MULTIPLE)) {
        ...
        foreach ($enrols as $courseid => $roles) {
            ...
            $current = $DB->get_records('role_assignments', array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_idatabase', 'itemid'=>$instance->id), '', 'id, roleid');
            ...
            foreach ($current as $r) {
                if (in_array($r->roleid, $roles)) {
                    ...
                    role_unassign($r->roleid, $user->id, $context->id, 'enrol_idatabase', $instance->id);
                }
            }
            foreach ($roles as $rid) {
                if (!isset($existing[$rid])) {
                    role_assign($rid, $user->id, $context->id, 'enrol_idatabase', $instance->id);
                }
            }
        }
        ...
        $sql = "SELECT e.*, c.visible AS cvisible, ue.status AS ustatus
                 ...
                 WHERE ue.userid = :userid AND e.enrol = 'idatabase'";
        ...
        foreach ($rs as $instance) {
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                    role_unassign_all(array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_idatabase', 'itemid'=>$instance->id));
                }
            }
        }
        $rs->close();
    ...
    public function sync_enrolments($verbose = false, $onecourse = null) {
        ...
        if ($onecourse) {
            $sql = "SELECT c.id, c.visible, c.$localcoursefield AS mapping, c.shortname, e.id AS enrolid
                      FROM {course} c
                 LEFT JOIN {enrol} e ON (e.courseid = c.id AND e.enrol = 'idatabase')
                     WHERE c.id = :id";
            ...
            // Course being restored is always hidden, we have to ignore the setting here.
            $ignorehidden = false;
            ...
            $sql = "SELECT c.id, c.visible, c.$localcoursefield AS mapping, e.id AS enrolid, c.shortname
                      FROM {course} c
                      JOIN {enrol} e ON (e.courseid = c.id AND e.enrol = 'idatabase')";
        ...
        foreach ($existing as $course) {
            ...
            $sql = "SELECT u.$localuserfield AS mapping, u.id, ue.status, ue.userid, ra.roleid
                      ...
                      JOIN {role_assignments} ra ON (ra.userid = u.id AND ra.itemid = ue.enrolid AND ra.component = 'enrol_idatabase')
                     WHERE u.deleted = 0";
            ...
            foreach ($requested_roles as $userid=>$userroles) {
                ...
                foreach ($userroles as $roleid) {
                    if (empty($current_roles[$userid][$roleid])) {
                        role_assign($roleid, $userid, $context->id, 'enrol_idatabase', $instance->id);
                        $current_roles[$userid][$roleid] = $roleid;
                        if ($verbose) {
                ...
                foreach($current_roles[$userid] as $cr) {
                    if (empty($userroles[$cr])) {
                        role_unassign($cr, $userid, $context->id, 'enrol_idatabase', $instance->id);
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                foreach ($current_status as $userid=>$status) {
                    ...
                    if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                        role_unassign_all(array('contextid'=>$context->id, 'userid'=>$userid, 'component'=>'enrol_idatabase', 'itemid'=>$instance->id));
                        if ($verbose) {
